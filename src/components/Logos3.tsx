  ...
  return (
    <section className="my-16 mx-auto md:-mb-16 -mb-28">
      <div className="pt-4 md:pt-6">
        <h2 className="md:text-[2.5rem] sm:text-[2rem] text-[1.5rem] font-semibold text-white text-shadow-200 tracking-wide text-center">{heading}</h2>
        <div className="flex items-center justify-center lg:max-w-5xl">
          <Carousel
            opts={{ loop: true }}
            plugins={[AutoScroll({ playOnInit: true })]}
          >
            <CarouselContent className="ml-0 space-x-8"> {/* Added space-x-8 for spacing */}
              {logos.map((logo) => (
                <CarouselItem
                  key={logo.id}
